...
{
    //画瓦斯涌出量点
    public class DrawWSYCLD
    {
        //////////////////////
        //图片
        public Bitmap m_Bitmap = new Bitmap(GIS.Properties.Resources.图片); //暂定使用程序内嵌图片，也可以自动生成图片，效果相同
        public string m_strJDWSYCL="5.4";//左侧分子
        public string m_strXDWSYCL = "8.4";//分子右侧
        public string m_strRCL = "3956";//分母左侧
        public string m_strTIME = "2003.02";//分母右侧
        //画笔
        public Pen m_pPen = new Pen(Color.Red);
        //写文字的字体,画刷和位置
        public Font m_pFont = new Font("宋体", 10);
        ...
        public DrawWSYCLD()
        {
            m_strJDWSYCL="5.4";//左侧分子
            m_strXDWSYCL = "8.4";//分子右侧
            m_strRCL = "3956";//分母左侧
            m_strTIME = "2003.02";//分母右侧
        ...
        private void DrawSymbol()
        {
            ...
            int width_ZuoShang = Convert.ToInt32(g.MeasureString(m_strJDWSYCL, m_pFont).Width);
            m_pBTWZPoint.X = 40 - width_ZuoShang;
            m_pBTWZPoint.Y = 12;
            g.DrawString(m_strJDWSYCL, m_pFont, m_pBrush, m_pBTWZPoint);

            //
            m_pBTWZPoint.X = 43;
            m_pBTWZPoint.Y = 12;
            g.DrawString(m_strXDWSYCL, m_pFont, m_pBrush, m_pBTWZPoint);


            //-600
            int width_ZuoXia = Convert.ToInt32(g.MeasureString(m_strRCL, m_pFont).Width);
            m_pBTWZPoint.X = 40 - width_ZuoXia;
            // m_pBTWZPoint.X = 8;
            m_pBTWZPoint.Y = 25;
            g.DrawString(m_strRCL, m_pFont, m_pBrush, m_pBTWZPoint);

            //620
            m_pBTWZPoint.X = 43;
            m_pBTWZPoint.Y = 25;
            g.DrawString(m_strTIME, m_pFont, m_pBrush, m_pBTWZPoint);
